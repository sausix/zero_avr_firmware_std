...
*
* Author: Lutz Harder, Adrian Sausenthaler
* License: GNU GPL
*
* Basic Firmware for ATmega32u4 slave,  no radio module support included
*
* Jul    15th, 2019  [00] v 1.1b initial Version (LH)
*
* October 4th, 2019  [00] v 1.2  Beta (LH)
*    NEW: implemented CRC Check via CRC8  CCITT
*
* May    04th, 2020  [01] v 2.0  Beta (AS)
*    NEW: Flash-Version check. Speed improvements.
*
*/
...
uint16_t data_lcd_A035VW01[] = {0x0080, 0x01F8,  0x0246, 0x0305, 0x0440,  0x0540, 0x0640,0x0740, 0x0840,  0x0940, 0x0A03};            // only for  A035VW01

uint16_t data_lcd_A035VL01[] = {0x0011,0x0000,0x0001,0x0000,0x00C1,0x01A8,0x01B1,0x0145,0x0104,0x0C5,0x0180,0x016C,0x0C6,               // initcode for LCD A035VL01
...
void setup_lcd(void) {
	//only for A035VW01
	for (int x=0; x<sizeof(data_lcd_A035VW01); x++) {
		write_lcd(data_lcd_A035VW01[x], 16);
	}


	//only for A035VL01
	for (int x=0; x<(sizeof(data_lcd_A035VL01) / sizeof(uint16_t)); x++) {
		if (data_lcd_A035VL01[x] == 0x0000) {
			_delay_ms(WAIT);
			continue;
		}
		write_lcd(data_lcd_A035VL01[x], 9);
	}
}
...
int main(void) {
  ...
  switch(adcselect)
  {
   ...
   default: {a7 = read_analog(10);  //read A7 more frequently
           if (a7 > a7max)
        	   a7max = a7;
           if (a7 < a7min)
        	   a7min = a7;
